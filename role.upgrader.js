        ...
        for( i = 0; i < Memory.base_profile.length; i++){
          ...
          for(j = 0; (j < (room_memory ? room_memory.length : 0) && (available_spawns.length != num_scheduled)); j++) {
            ...
            for(k in room_memory[j].room_energy) {
                var energy_miner_bot = _.filter(Game.creeps, (creep) => (creep.memory.resource_id == room_memory[j].room_energy[k].energy_id) && (creep.memory.role == 'energy_miner') && (creep.ticksToLive > 20));
                //console.log('Creep body cost: ' +role_creator.bodypartcost(energy_miner_bot[0].body));
              if(energy_miner_bot.length < 1) role_creator.build_energy_miner(i, room_index, k, available_spawns[num_scheduled++].spawn_id);
              if(num_scheduled == available_spawns.length) break;

                var energy_transport_bot = _.filter(Game.creeps, (creep) => (creep.memory.resource_id == room_memory[j].room_energy[k].energy_id) && (creep.memory.role == 'energy_transport') && (creep.ticksToLive > 20));
              if(energy_transport_bot.length < 1){ console.log('trying to build a transport unit'); role_creator.build_energy_transport(i, room_index, k, available_spawns[num_scheduled++].spawn_id);}
              if(num_scheduled == available_spawns.length) break;
            }
